...
public class DropApp extends GameApplication {
    ...
    protected void initPhysics() {
        onCollisionBegin(Type.BUCKET, Type.DROPLET, (_, droplet) -> {
            ...
            // increase score by 1 for catching a droplet
            score++;
            updateScoreDisplay();
    ...
    protected void onUpdate(double tpf) {
        ...
        getGameWorld().getEntitiesByType(Type.DROPLET).forEach(droplet -> {
            ...
            // check if droplet has fallen off the bottom of the screen
            if (droplet.getY() > getAppHeight()) {
                // remove the missed droplet
                droplet.removeFromWorld();
                
                // decrease score by 1 for missing a droplet
                score--;
                updateScoreDisplay();
            }
        });
    ...
    private void spawnBucket() {
        ...
        Entity bucket = entityBuilder()
                .type(Type.BUCKET)
                .at(getAppWidth() / 2, getAppHeight() - 50)
